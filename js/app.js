...
function buildBookCard(book, extraClasses = '') {
  ...
  return `
    <div class="book-card bg-white rounded-2xl overflow-hidden shadow-md ${extraClasses}" data-id="${book.id}">
      <!-- Cover -->
      <div class="book-cover-wrap ${book.coverClass} relative" style="height:200px; background-image: url('${book.cover}'); background-size: cover; background-position: center;">
        <img src="${book.cover}" alt="${book.title}" style="width:100%; height:100%; object-fit:cover; display:none;" onerror="this.style.display='none';" />
        <div class="absolute inset-0 flex flex-col items-center justify-center p-4 text-white text-center" style="background: ${book.cover ? 'none' : 'linear-gradient(135deg, #f59e0b 0%, #d97706 100%)'}; display: ${book.cover ? 'none' : 'flex'};">
...
function buildUsedBookCard(book) {
  ...
  return `
    <div class="used-book-card book-card bg-white rounded-2xl overflow-hidden shadow-md border-2 border-dashed border-amber-200" data-used-id="${book.id}">
      <!-- Cover -->
      <div class="book-cover-wrap ${book.coverClass} relative" style="height:180px; background-image: url('${book.cover}'); background-size: cover; background-position: center;">
        <img src="${book.cover}" alt="${book.title}" style="width:100%; height:100%; object-fit:cover; display:none;" onerror="this.style.display='none';" />
        <div class="absolute inset-0 flex flex-col items-center justify-center p-4 text-white text-center" style="background: ${book.cover ? 'none' : 'linear-gradient(135deg, #f59e0b 0%, #d97706 100%)'}; display: ${book.cover ? 'none' : 'flex'};">
